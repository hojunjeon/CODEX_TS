...
def _compact_git_diff(text: str) -> str:
    ...
    for line in text.splitlines():
        if line.startswith("diff --git "):
            ...
            hunks.append(line.strip() + suffix)
...
def _compact_search(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        hits_by_file.setdefault(path, []).append(f"{line_no}:{body.strip()[:140]}")
...
def _compact_listing(text: str) -> str:
    ...
    kept = [f"listing: {len(lines)} entries", *interesting[:80], f"[ccts] omitted {max(0, len(lines) - len(interesting[:80]))} low-signal entries."]
    return _dedupe_lines(kept)
...
def _compact_build_lint(text: str) -> str:
    ...
    if len(interesting) < len(lines):
        kept.append(f"[ccts] omitted {len(lines) - len(interesting)} low-signal lines.")
    return _dedupe_lines(kept)
...
def _compact_generic(text: str) -> str:
    ...
    if len(lines) <= 24 and not interesting:
        return text.strip()
    kept = ["[ccts:generic] compacted output", *lines[:6], *interesting[:24], "...", *lines[-6:]]
...
def _dedupe_with_omission(lines: list[str], original_line_count: int) -> str:
    ...
    if omitted:
        out.append(f"[ccts] omitted {omitted} low-signal lines.")
    return "\n".join(out).strip() + "\n"
